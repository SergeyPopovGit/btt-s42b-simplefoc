...
void ManualControl::loop(){
  ...
  if(!HAL_GPIO_ReadPin(MANUAL_CONTROL_GPIO_PORT, INC_PIN) ){
      ...
      if(upTime>0) cur_increment = 0.01  ; //set increment to 
      if(upTime>5) cur_increment = 0.1  ; //set increment to
      if(upTime>10) cur_increment = 1  ; //set increment to
  ...
  if(!HAL_GPIO_ReadPin(MANUAL_CONTROL_GPIO_PORT, DEC_PIN) ){
      ...
      if(downTime>0) cur_increment = -0.01  ; //set increment to 
      if(downTime>5) cur_increment = -0.1  ; //set increment to
      if(downTime>10) cur_increment = -1  ; //set increment to
