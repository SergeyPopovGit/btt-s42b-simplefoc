...
float StepperMotor::correctionTable[_POLE_PAIRS] ;  //<! calibration table  of pole angle value 
...
int StepperMotor::alignSensor() {
  ...
  if(!_isset(zero_electric_angle)){
    ...
    // set angle -90(270 = 3PI/2) degrees 
     ...
     // DBGPrintPoleCoord(); //print debug information of pole coord
     fillCorrectionTable();  //fill correction table 
     
     //zero_electric_angle = correctionTable[1] ; //set zero correction from table
...
float dbg_electrical_angle = 0 ;
float dbg_compensate_angle = 0 ;
float dbg_shaft_angle = 0;
float dbg_pole_angle = 0;
int dbg_index = 0;
float dbg_voltage_q = 0 ;

/*
* compensate deviation of zero electrical angle
*  by correction table 
*/
float StepperMotor::getElectricalAngle(float _shaft_angle){
    //calculate index of corraction table
  int index = floor(_shaft_angle*(_POLE_PAIRS/_2PI));
    //normalize index
   index = index  >= 0 ? index  : (index  + _POLE_PAIRS);

  return _normalizeAngle(_electricalAngle(_shaft_angle-correctionTable[index],pole_pairs));
  //debug part
    //dbg_compensate_angle = zero_electric_angle;
    //dbg_index = index;
    //dbg_shaft_angle = shaft_angle;
    //dbg_pole_angle = 
}
...
/**************
 * function for make step correction table of stepper motor
 *
 * 
 */
    //  function of get average angle by 10 mesurement
    //  start angle can set for continue averaging 

  float StepperMotor::getAverageAngle(float start_angle = NOT_SET){
      float delta_angle = 0 ; //delta value from first read
        //if start angle hasn't set
      if (start_angle == NOT_SET) start_angle = sensor->getAngle() ; //set start angle
      float accum_delta = 0 ;  //accumulate normalize delta value
      int i=1 ;  // iteration number
      while( i <= 10 ){
            //calculate  delta
          delta_angle =  sensor->getAngle() - start_angle;
            //normalize delta
          if(delta_angle >  _PI) delta_angle -= _2PI; 
          if(delta_angle < -_PI) delta_angle += _2PI;
          accum_delta += delta_angle;          
          _delay(2);
          i++;
        };
      
      return fmod(start_angle+accum_delta/i,_2PI); //return normalize averaga angle 
  }

    /*
    * print debug information
    */
  void StepperMotor::dbg_print (int number, float value){
      if(monitor_port&&_PRINT_DEBUG){ 
       monitor_port->print(number); //print number of table index
       monitor_port->print(F(" ; ")); //delimeter
       monitor_port->println(value,5); //print angle value
      };
  };

   /* 
   * function for filling correction table
   */ 
  void StepperMotor::fillCorrectionTable(){
      
    float angle ; //value of motor angle in step  
    ...
    while ( i <= pole_pairs*4 ) {
        ...
        if(i%4 == 0){  //every 4 index 
          correctionTable[k] = getAverageAngle() ; //get average angle 
          dbg_print(k,correctionTable[k]); //print debug information
          k++;  
          };
        i++ ;
      };
      ...
      _delay(250);
      //---------------------------
      //do backward rotate in full step mode
      //rotate by PI/2 step size
      i--;  //
      k--;  
    while ( i >= 1  ) {
        ...
        if(i%4 == 0){  //every 4 index 
          correctionTable[k] = getAverageAngle(correctionTable[k]) ; // average angle  
          dbg_print(k,correctionTable[k]); //print debug information
          k--;  
        };
        i-- ;
...
void StepperMotor::DBGPrintPoleCoord(){
    //downside_z_angle
    float z_angle = 0;

    if(monitor_port){ 
     monitor_port->println(F("MOT_DBG: Start calib."));

     for (int i = 0; i <=200; i++ ) {
      float angle = _PI_2 * i+_PI ;
      setPhaseVoltage(voltage_sensor_align, 0,  angle);
      _delay(500);
     //+++++++
      
      monitor_port->print(i);
      monitor_port->print(F(" ; "));
        //
      z_angle = 0;
      for(int i=0 ; i < 10 ;i++){
          z_angle += sensor->getAngle();
          _delay(2);
        };
      monitor_port->println(z_angle/10,5);
     };

    monitor_port->println(F("MOT_DBG: Back."));
