...
double getAngle(bool average) {
    ...
    double angle = ((RADS_IN_CIRCLE / POW_2_15) * ((double) rawData));
...
double getEncoderSpeed() {
	...
	rawAngleRange >>= 4;
	double angleRange = RADS_IN_CIRCLE * (POW_2_7 / (double) (rawAngleRange));
    ...
    // rad/s
    return ((angleRange / POW_2_15) * ((double) rawSpeed)) / (((double) intMode2Prediction) * firMDVal * 0.000001);
}
